...
def main():
    outdir, genus, level, overwrite = parsing_arguments()
    ...
    if not os.path.exists(f"assembly_summary_{month}.txt"):
        ...
        os.system(f"rsync --copy-links --times --verbose rsync://ftp.ncbi.nlm.nih.gov/genomes/refseq/bacteria/assembly_summary.txt ./assembly_summary_{month}.txt")
    else:
    ...
    ftp_list = filter_assembly(file = f"assembly_summary_{month}.txt", a_level = level, genus = genus, date = month)
    download_gcf(ftp_list, overwrite)
    make_info_file(f"assembly_summary_{month}.txt", ftp_list)
...
def parsing_arguments():
    ...
    parser.add_argument('--level-of-assembly', dest='level', help = "Level of assembly for download", choices = ['Complete Genome', 'Chromosome', 'Contig', 'Scaffold'], default = "Complete Genome")
    ...
    return outdir, genus, level, overwrite
